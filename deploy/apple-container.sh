...
usage() {
    cat <<'EOF'
用法：./apple-container.sh <命令> [选项]

命令：
  init                  创建 .env 并生成必需密钥
  up [--recreate]       创建并启动完整的 Sub2API Plus 服务栈
  down                  停止服务栈并保留全部数据
  restart               按依赖顺序重启服务栈
  status                显示容器状态并执行健康检查
  logs <service> [-f]   查看 app、postgres 或 redis 日志
  pull                  拉取全部 linux/arm64 镜像
  destroy [选项]        删除服务栈容器和网络
...
环境变量：
  SUB2API_ENV_FILE      部署环境文件路径（默认：deploy/.env）
EOF
}
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_SUB2API_IMAGE ghcr.io/itv3/sub2apiplus:latest)"
