...
const paymentGuideHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/itv3/sub2apiplus/blob/main/docs/PAYMENT_CN.md"
    : "https://github.com/itv3/sub2apiplus/blob/main/docs/PAYMENT.md",
);

const paymentMethodsHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/itv3/sub2apiplus/blob/main/docs/PAYMENT_CN.md#支持的支付方式"
    : "https://github.com/itv3/sub2apiplus/blob/main/docs/PAYMENT.md#supported-payment-methods",
);
...
const form = reactive<SettingsForm>({
  ...
  // 只读展示：自动发现值与 strict wire 已验收值均不参与提交。
  openai_codex_client_version_synced: "",
  openai_codex_verified_client_version: "",
...
const codexVerifiedVersionLabel = computed(() => {
  const verified = form.openai_codex_verified_client_version?.trim();
  if (!verified) return "";
  return t("admin.settings.gatewayForwarding.openaiCodexVerifiedVersionValue", {
    version: verified,
  });
});
