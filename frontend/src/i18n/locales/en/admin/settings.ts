export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentHint: 'Candidate User-Agent input. It contributes only the client name and OS / architecture / terminal fingerprint; its version is rebuilt from the candidate version. Strict OAuth egress is still finalized by the verified ReleaseBundle.',
        openaiCodexClientVersion: 'Codex candidate version',
        openaiCodexClientVersionPlaceholder: 'Leave empty to follow discovery',
        openaiCodexClientVersionHint: 'Used for discovery, compatibility checks, and preparing a future persona upgrade. It never activates strict wire automatically; a new version still requires evidence and all 42 acceptance checks.',
        openaiCodexVersionAutoSync: 'Discover latest Codex version',
        openaiCodexVersionAutoSyncHint: 'Discovers the latest stable release every 6 hours with list-scan fallback. The discovered value is only an upgrade candidate and never changes the active strict persona automatically.',
        openaiCodexVersionSyncedValue: 'Latest discovered: {version}',
        openaiCodexVerifiedVersionValue: 'Strict wire active: {version}',
