...
const codex0151R9CandidateReadinessSourceTransitionPath = "docs/egress/maintenance/codex-cli-0151-r9-candidate-readiness-source-transition.json"

var (
	codex0151R9CandidateReadinessSourceTransitionOnce   sync.Once
	codex0151R9CandidateReadinessSourceTransitionCached codex0151ToolReadinessReceipt
	codex0151R9CandidateReadinessSourceTransitionErr    error
)

func loadCodex0151R9CandidateReadinessSourceTransition() (codex0151ToolReadinessReceipt, error) {
	codex0151R9CandidateReadinessSourceTransitionOnce.Do(func() {
		codex0151R9CandidateReadinessSourceTransitionCached,
			codex0151R9CandidateReadinessSourceTransitionErr =
			readCodex0151R9CandidateReadinessSourceTransition()
	})
	return codex0151R9CandidateReadinessSourceTransitionCached,
		codex0151R9CandidateReadinessSourceTransitionErr
}

func readCodex0151R9CandidateReadinessSourceTransition() (codex0151ToolReadinessReceipt, error) {
	var receipt codex0151ToolReadinessReceipt
	raw, err := os.ReadFile(codex01491TerminalRepoPath(codex0151R9CandidateReadinessSourceTransitionPath))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateCodex0151R9CandidateReadinessSourceTransition(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151R9CandidateReadinessSourceTransition(receipt codex0151ToolReadinessReceipt) error {
	...
	if receipt.Predecessor.Kind != "codex_cli_0151_historical_rehearsal_successor_source_transition" ||
		receipt.Predecessor.Path != codex0151HistoricalRehearsalSuccessorSourceTransitionPath {
	...
	for _, transition := range receipt.Transitions {
		...
		if readErr != nil || (currentDigest != transition.ToSHA256 &&
			!codex0151Arm64EnvironmentProducerReplayToolSuccessorSupersedes(
				transition.Path,
				transition.ToSHA256,
				currentDigest,
			)) {
...
func codex0151R9CandidateReadinessSourceTransitionSupersedes(path, priorDigest, currentDigest string) bool {
	receipt, err := loadCodex0151R9CandidateReadinessSourceTransition()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
	...
	return codex0151Arm64EnvironmentProducerReplayToolSuccessorSupersedes(
		path,
		priorDigest,
		currentDigest,
	)
...
func TestCodex0151R9CandidateReadinessSourceTransitionIsFrozen(t *testing.T) {
	if _, err := loadCodex0151R9CandidateReadinessSourceTransition(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151R9CandidateReadinessSourceTransitionRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151R9CandidateReadinessSourceTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateCodex0151R9CandidateReadinessSourceTransition(mutated); err == nil {
