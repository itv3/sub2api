...
const codex0151ModelPolicyToolSuccessorPath = "docs/egress/maintenance/codex-cli-0151-model-policy-tool-successor-source-transition.json"

var (
	codex0151ModelPolicyToolSuccessorOnce   sync.Once
	codex0151ModelPolicyToolSuccessorCached codex0151ToolReadinessReceipt
	codex0151ModelPolicyToolSuccessorErr    error
)

func loadCodex0151ModelPolicyToolSuccessor() (codex0151ToolReadinessReceipt, error) {
	codex0151ModelPolicyToolSuccessorOnce.Do(func() {
		codex0151ModelPolicyToolSuccessorCached, codex0151ModelPolicyToolSuccessorErr =
			readCodex0151ModelPolicyToolSuccessor()
	})
	return codex0151ModelPolicyToolSuccessorCached, codex0151ModelPolicyToolSuccessorErr
}

func readCodex0151ModelPolicyToolSuccessor() (codex0151ToolReadinessReceipt, error) {
	var receipt codex0151ToolReadinessReceipt
	raw, err := os.ReadFile(codex01491TerminalRepoPath(codex0151ModelPolicyToolSuccessorPath))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateCodex0151ModelPolicyToolSuccessor(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151ModelPolicyToolSuccessor(receipt codex0151ToolReadinessReceipt) error {
	...
	if receipt.Predecessor.Kind != "codex_cli_0151_c2pa_capture_tool_successor_source_transition" ||
		receipt.Predecessor.Path != codex0151C2PACaptureToolSuccessorPath ||
...
// codex0151ModelPolicyToolSuccessorSupersedes 只承接模型政策工具后继的精确摘要边。
func codex0151ModelPolicyToolSuccessorSupersedes(path, priorDigest, currentDigest string) bool {
	receipt, err := loadCodex0151ModelPolicyToolSuccessor()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
...
func TestCodex0151ModelPolicyToolSuccessorSourceTransitionIsFrozen(t *testing.T) {
	if _, err := loadCodex0151ModelPolicyToolSuccessor(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151ModelPolicyToolSuccessorSourceTransitionRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151ModelPolicyToolSuccessor()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			mutated := receipt
			test.mutate(&mutated)
			if err := validateCodex0151ModelPolicyToolSuccessor(mutated); err == nil {
