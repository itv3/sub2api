		...
		case "present":
			...
			if readErr != nil || len(transition.CurrentSHA256) != 64 ||
				(currentDigest != transition.CurrentSHA256 &&
					...
					transition.CurrentSHA256,
					currentDigest,
				) && !openAIWSEmptyTerminalOutputRepairSupersedes(
					transition.Path,
					transition.CurrentSHA256,
					currentDigest,
...
func readOpenAIReplayOOMRepairTransition() (openAIReplayOOMRepairReceipt, error) {
	receipt, err := readOpenAIReplayOOMRepairTransitionRaw()
	if err != nil {
		return receipt, err
	}
...
// readOpenAIReplayOOMRepairTransitionRaw 只读取并校验收据自身的编码与摘要，
// 不递归校验其依赖链。依赖链校验期间只能使用这个无递归读取器，避免
// sync.Once 正在加载当前收据时再次等待自身。
func readOpenAIReplayOOMRepairTransitionRaw() (openAIReplayOOMRepairReceipt, error) {
...
func readOpenAIWSCompatibilityGuardRepairTransition() (
	...
	receipt, err := readOpenAIWSCompatibilityGuardRepairTransitionRaw()
	if err != nil {
		return receipt, err
	}
...
// readOpenAIWSCompatibilityGuardRepairTransitionRaw 只读取收据本身，供依赖链
// 比对使用；它不调用任何前序 transition 验证器，保证加载过程无递归等待。
func readOpenAIWSCompatibilityGuardRepairTransitionRaw() (
	openAIWSCompatibilityGuardRepairReceipt,
	error,
) {
...
func openAIWSCompatibilityGuardRepairSupersedes(
	...
	predecessor, err := readOpenAIReplayOOMRepairTransitionRaw()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
...
const openAIWSEmptyTerminalOutputRepairTransitionPath = "docs/egress/maintenance/openai-ws-empty-terminal-output-repair-source-transition.json"

type openAIWSEmptyTerminalOutputRepairReceipt struct {
	SchemaVersion  string                            `json:"schema_version"`
	IssuedAtUTC    string                            `json:"issued_at_utc"`
	BaseCommit     string                            `json:"base_commit"`
	Scope          string                            `json:"scope"`
...
var (
	openAIWSEmptyTerminalOutputRepairOnce    sync.Once
	openAIWSEmptyTerminalOutputRepairCached  openAIWSEmptyTerminalOutputRepairReceipt
	openAIWSEmptyTerminalOutputRepairLoadErr error
)

func loadOpenAIWSEmptyTerminalOutputRepairTransition() (
	openAIWSEmptyTerminalOutputRepairReceipt,
	error,
) {
	openAIWSEmptyTerminalOutputRepairOnce.Do(func() {
		openAIWSEmptyTerminalOutputRepairCached,
			openAIWSEmptyTerminalOutputRepairLoadErr =
			readOpenAIWSEmptyTerminalOutputRepairTransition()
	})
	return openAIWSEmptyTerminalOutputRepairCached,
		openAIWSEmptyTerminalOutputRepairLoadErr
}

func readOpenAIWSEmptyTerminalOutputRepairTransition() (
	openAIWSEmptyTerminalOutputRepairReceipt,
	error,
) {
	receipt, err := readOpenAIWSEmptyTerminalOutputRepairTransitionRaw()
	if err != nil {
		return receipt, err
	}
	if err := validateOpenAIWSEmptyTerminalOutputRepairTransition(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

// readOpenAIWSEmptyTerminalOutputRepairTransitionRaw 只读取收据自身，供依赖链
// 比对使用；它不触发前序 transition 的 sync.Once 加载。
func readOpenAIWSEmptyTerminalOutputRepairTransitionRaw() (
	openAIWSEmptyTerminalOutputRepairReceipt,
	error,
) {
	var receipt openAIWSEmptyTerminalOutputRepairReceipt
	raw, err := os.ReadFile(codex01491TerminalRepoPath(
		openAIWSEmptyTerminalOutputRepairTransitionPath,
	))
	if err != nil {
		return receipt, err
	}
...
func validateOpenAIWSEmptyTerminalOutputRepairTransition(
	receipt openAIWSEmptyTerminalOutputRepairReceipt,
) error {
	...
	if receipt.Predecessor.Kind != "openai_ws_compatibility_guard_repair_source_transition" ||
		receipt.Predecessor.Path != openAIWSCompatibilityGuardRepairTransitionPath ||
...
func openAIWSEmptyTerminalOutputRepairSupersedes(
	path string,
	priorDigest string,
	currentDigest string,
) bool {
...
func TestOpenAIWSEmptyTerminalOutputRepairSourceTransitionIsFrozen(t *testing.T) {
	if _, err := loadOpenAIWSEmptyTerminalOutputRepairTransition(); err != nil {
		t.Fatal(err)
	}
}

const codex0151ToolReadinessTransitionPath = "docs/egress/maintenance/codex-cli-0151-tool-readiness-source-transition.json"

type codex0151ToolReadinessReceipt struct {
	SchemaVersion  string                            `json:"schema_version"`
	IssuedAtUTC    string                            `json:"issued_at_utc"`
	BaseCommit     string                            `json:"base_commit"`
	Scope          string                            `json:"scope"`
...
var (
	codex0151ToolReadinessOnce    sync.Once
	codex0151ToolReadinessCached  codex0151ToolReadinessReceipt
	codex0151ToolReadinessLoadErr error
)

func loadCodex0151ToolReadinessTransition() (codex0151ToolReadinessReceipt, error) {
	codex0151ToolReadinessOnce.Do(func() {
		codex0151ToolReadinessCached, codex0151ToolReadinessLoadErr =
			readCodex0151ToolReadinessTransition()
	})
	return codex0151ToolReadinessCached, codex0151ToolReadinessLoadErr
}

func readCodex0151ToolReadinessTransition() (codex0151ToolReadinessReceipt, error) {
	var receipt codex0151ToolReadinessReceipt
	raw, err := os.ReadFile(codex01491TerminalRepoPath(codex0151ToolReadinessTransitionPath))
	if err != nil {
		return receipt, err
	}
	decoder := json.NewDecoder(bytes.NewReader(raw))
	...
	if err := validateCodex0151ToolReadinessTransition(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151ToolReadinessTransition(receipt codex0151ToolReadinessReceipt) error {
	...
	if receipt.Predecessor.Kind != "openai_ws_empty_terminal_output_repair_source_transition" ||
		receipt.Predecessor.Path != openAIWSEmptyTerminalOutputRepairTransitionPath ||
	...
	for _, transition := range receipt.Transitions {
		...
		currentDigest := upstreamMergeFrameworkDigest(current)
		if readErr != nil || (currentDigest != transition.ToSHA256 &&
			!codex0151C2PACaptureToolSuccessorSupersedes(
				transition.Path,
				transition.ToSHA256,
				currentDigest,
			)) {
...
// codex0151ToolReadinessTransitionSupersedes 只承接本次工具就绪变更的精确摘要边。
func codex0151ToolReadinessTransitionSupersedes(path, priorDigest, currentDigest string) bool {
	receipt, err := loadCodex0151ToolReadinessTransition()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
	...
	return codex0151C2PACaptureToolSuccessorSupersedes(path, priorDigest, currentDigest)
}

func TestCodex0151ToolReadinessSourceTransitionIsFrozen(t *testing.T) {
	if _, err := loadCodex0151ToolReadinessTransition(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151ToolReadinessSourceTransitionRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151ToolReadinessTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			mutated := receipt
			test.mutate(&mutated)
			if err := validateCodex0151ToolReadinessTransition(mutated); err == nil {
				t.Fatal("变异后的工具就绪 transition 被错误接受")
			}
		})
	}
}
