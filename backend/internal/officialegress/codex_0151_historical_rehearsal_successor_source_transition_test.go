...
const codex0151HistoricalRehearsalSuccessorSourceTransitionPath = "docs/egress/maintenance/codex-cli-0151-historical-rehearsal-successor-source-transition.json"

var (
	codex0151HistoricalRehearsalSuccessorSourceTransitionOnce   sync.Once
	codex0151HistoricalRehearsalSuccessorSourceTransitionCached codex0151ToolReadinessReceipt
	codex0151HistoricalRehearsalSuccessorSourceTransitionErr    error
)

func loadCodex0151HistoricalRehearsalSuccessorSourceTransition() (codex0151ToolReadinessReceipt, error) {
	codex0151HistoricalRehearsalSuccessorSourceTransitionOnce.Do(func() {
		codex0151HistoricalRehearsalSuccessorSourceTransitionCached,
			codex0151HistoricalRehearsalSuccessorSourceTransitionErr =
			readCodex0151HistoricalRehearsalSuccessorSourceTransition()
	})
	return codex0151HistoricalRehearsalSuccessorSourceTransitionCached,
		codex0151HistoricalRehearsalSuccessorSourceTransitionErr
}

func readCodex0151HistoricalRehearsalSuccessorSourceTransition() (codex0151ToolReadinessReceipt, error) {
	var receipt codex0151ToolReadinessReceipt
	raw, err := os.ReadFile(codex01491TerminalRepoPath(codex0151HistoricalRehearsalSuccessorSourceTransitionPath))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateCodex0151HistoricalRehearsalSuccessorSourceTransition(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151HistoricalRehearsalSuccessorSourceTransition(receipt codex0151ToolReadinessReceipt) error {
	...
	if receipt.Predecessor.Kind != "codex_cli_0151_deterministic_package_fixture_source_transition" ||
		receipt.Predecessor.Path != codex0151DeterministicPackageFixtureSourceTransitionPath {
	...
	for _, transition := range receipt.Transitions {
		...
		if readErr != nil || (currentDigest != transition.ToSHA256 &&
			!codex0151R9CandidateReadinessSourceTransitionSupersedes(
				transition.Path,
				transition.ToSHA256,
				currentDigest,
			)) {
...
func codex0151HistoricalRehearsalSuccessorSourceTransitionSupersedes(path, priorDigest, currentDigest string) bool {
	if codex0151CurrentSourceDigestAccepted(path, priorDigest, currentDigest) {
		return true
	}
	receipt, err := loadCodex0151HistoricalRehearsalSuccessorSourceTransition()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
	...
	return codex0151R9CandidateReadinessSourceTransitionSupersedes(
		path,
		priorDigest,
		currentDigest,
	)
...
func TestCodex0151HistoricalRehearsalSuccessorSourceTransitionIsFrozen(t *testing.T) {
	if _, err := loadCodex0151HistoricalRehearsalSuccessorSourceTransition(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151HistoricalRehearsalSuccessorSourceTransitionRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151HistoricalRehearsalSuccessorSourceTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateCodex0151HistoricalRehearsalSuccessorSourceTransition(mutated); err == nil {
