...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	nativeV2 := isBareOpenAIResponsesPath(c) && isOpenAIRemoteCompactionV2Request(c, body, compactionProfileMode)
	if nativeV2 {
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	ctx := service.WithOpenAIAPIKeyMimicRequestContext(clientLifecycleCtx, c)
	...
	for {
		...
		if preemptCtx, cleanupPreempt, armed := h.gatewayService.BeginOpenAIWSIngressSessionPreemption(accountCtx, c, account, wsFirstMessage); armed {
			ctx = preemptCtx
