...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.OpenAICodexClientVersion != nil {
		// 候选值会进入 UA/version 兼容验证，必须是合法版本号；空串表示跟随自动发现。
		normalized := strings.TrimSpace(*req.OpenAICodexClientVersion)
		if normalized != "" && service.NormalizeCodexClientVersion(normalized) == "" {
			response.Error(c, http.StatusBadRequest, "openai_codex_client_version must be empty or a valid semantic version")
			return
		}
	...
	payload := dto.SystemSettings{
		...
		OpenAICodexVerifiedClientVersion:                       updatedSettings.OpenAICodexVerifiedClientVersion,
