...
type GatewayOpenAIWSConfig struct {
	...
	// ClientFirstMessageTimeoutSeconds 限制 WebSocket 升级后读取并解压首条客户端 response.create 消息的总时长。
	ClientFirstMessageTimeoutSeconds int `mapstructure:"client_first_message_timeout_seconds"`
	// IngressInterTurnIdleTimeoutSeconds 限制客户端在两次已完成的入站请求之间可保持空闲的时长。
	// 设置为 0 时关闭此项保护。
	IngressInterTurnIdleTimeoutSeconds int `mapstructure:"ingress_inter_turn_idle_timeout_seconds"`
	// MaxIngressConnectionsPerAPIKey 限制每个 API 密钥在所有实例上的客户端 WebSocket 入站会话总数。
	// 设置为 0 时关闭此项保护。
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	// 兼容旧部署：v0.1.161 之前只有 max_body_size。若旧值低于新增的 32 MiB
	// 默认文本上限，且管理员未显式配置新键，则自动收敛到旧总上限。
	if !hasExplicitConfigOrEnv("gateway.text_max_body_size", "GATEWAY_TEXT_MAX_BODY_SIZE") &&
...
func (c *Config) Validate() error {
	...
	switch strings.ToLower(strings.TrimSpace(c.Gateway.OfficialClientProfiles.Mode)) {
