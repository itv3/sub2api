...
func TestLoadTextBodyLimitCompatibility(t *testing.T) {
	t.Run("未配置新键时沿用较小的旧总上限", func(t *testing.T) {
		resetViperWithJWTSecret(t)
		t.Setenv("GATEWAY_MAX_BODY_SIZE", "16777216")

		cfg, err := Load()
		require.NoError(t, err)
		require.Equal(t, int64(16*1024*1024), cfg.Gateway.MaxBodySize)
		require.Equal(t, cfg.Gateway.MaxBodySize, cfg.Gateway.TextMaxBodySize)
	})

	t.Run("显式配置的新键超过总上限时仍拒绝启动", func(t *testing.T) {
		resetViperWithJWTSecret(t)
		t.Setenv("GATEWAY_MAX_BODY_SIZE", "16777216")
		t.Setenv("GATEWAY_TEXT_MAX_BODY_SIZE", "33554432")

		_, err := Load()
		require.ErrorContains(t, err, "gateway.text_max_body_size")
	})
}
...
func TestValidateConfigErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "h2c oversized connection upload buffer",
			mutate: func(c *Config) {
				c.Server.H2C.Enabled = true
		...
		{
			name: "h2c oversized stream upload buffer",
			mutate: func(c *Config) {
				c.Server.H2C.Enabled = true
				c.Server.H2C.MaxUploadBufferPerStream = int(math.MaxInt32) + 1
			},
