...
func TestGatewayService_AnthropicAPIKeyMimicSkipsOfficialClaudeCodeRequest(t *testing.T) {
	...
	c.Request.Header.Set("x-client-request-id", "official-request-id")
	...
	require.Equal(t, "official-request-id", getHeaderRaw(req.Header, "x-client-request-id"))
...
func TestGatewayService_AnthropicSetupTokenMimic_RewritesSystemWithBillingBlock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			arr := system.Array()
			require.Len(t, arr, 4, "官方画像应包含 billing、身份、主提示词和文本输出边界")
			...
			require.Equal(t, officialAnthropicSystemSplitMarker, arr[3].Get("text").String())
			require.Equal(t, "1h", arr[3].Get("cache_control.ttl").String())
			...
			require.Equal(t, "30130130-1301-4301-8301-301301301301", metadata.AccountUUID)
		})
	}
}
...
func TestGatewayService_AnthropicSetupToken_LegacySystemSettingDoesNotDisableBuiltInProfile(t *testing.T) {
	...
	require.Contains(t, string(upstream.lastBody), "Original system prompt")
}
