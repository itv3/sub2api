...
const codex0151StoppedLedgerRecoverySourceTransitionServicePath = "docs/egress/maintenance/codex-cli-0151-stopped-ledger-recovery-source-transition.json"

var (
	codex0151StoppedLedgerRecoverySourceTransitionServiceOnce   sync.Once
	codex0151StoppedLedgerRecoverySourceTransitionServiceCached codex0151ToolReadinessReceiptService
	codex0151StoppedLedgerRecoverySourceTransitionServiceErr    error
)

func loadCodex0151StoppedLedgerRecoverySourceTransitionService() (codex0151ToolReadinessReceiptService, error) {
	codex0151StoppedLedgerRecoverySourceTransitionServiceOnce.Do(func() {
		codex0151StoppedLedgerRecoverySourceTransitionServiceCached,
			codex0151StoppedLedgerRecoverySourceTransitionServiceErr =
			readCodex0151StoppedLedgerRecoverySourceTransitionService()
	})
	return codex0151StoppedLedgerRecoverySourceTransitionServiceCached,
		codex0151StoppedLedgerRecoverySourceTransitionServiceErr
}

func readCodex0151StoppedLedgerRecoverySourceTransitionService() (codex0151ToolReadinessReceiptService, error) {
	var receipt codex0151ToolReadinessReceiptService
	raw, err := os.ReadFile(filepath.Join("../../..", filepath.FromSlash(codex0151StoppedLedgerRecoverySourceTransitionServicePath)))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateCodex0151StoppedLedgerRecoverySourceTransitionService(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151StoppedLedgerRecoverySourceTransitionService(receipt codex0151ToolReadinessReceiptService) error {
	...
	for _, transition := range receipt.Transitions {
		...
		if readErr != nil || (currentDigest != transition.ToSHA256 &&
			!codex0151DeterministicPackageFixtureSourceTransitionSupersedesService(
				transition.Path,
				transition.ToSHA256,
				currentDigest,
			)) {
...
func codex0151StoppedLedgerRecoverySourceTransitionSupersedesService(path, priorDigest, currentDigest string) bool {
	...
	return codex0151DeterministicPackageFixtureSourceTransitionSupersedesService(
		path,
		priorDigest,
		currentDigest,
	)
...
func TestCodex0151StoppedLedgerRecoverySourceTransitionServiceIsFrozen(t *testing.T) {
	if _, err := loadCodex0151StoppedLedgerRecoverySourceTransitionService(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151StoppedLedgerRecoverySourceTransitionServiceRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151StoppedLedgerRecoverySourceTransitionService()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateCodex0151StoppedLedgerRecoverySourceTransitionService(mutated); err == nil {
