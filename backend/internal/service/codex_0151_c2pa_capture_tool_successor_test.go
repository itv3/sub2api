...
const codex0151C2PACaptureToolSuccessorServicePath = "docs/egress/maintenance/codex-cli-0151-c2pa-capture-tool-successor-source-transition.json"

var (
	codex0151C2PACaptureToolSuccessorServiceOnce   sync.Once
	codex0151C2PACaptureToolSuccessorServiceCached codex0151ToolReadinessReceiptService
	codex0151C2PACaptureToolSuccessorServiceErr    error
)

func loadCodex0151C2PACaptureToolSuccessorService() (codex0151ToolReadinessReceiptService, error) {
	codex0151C2PACaptureToolSuccessorServiceOnce.Do(func() {
		codex0151C2PACaptureToolSuccessorServiceCached, codex0151C2PACaptureToolSuccessorServiceErr =
			readCodex0151C2PACaptureToolSuccessorService()
	})
	return codex0151C2PACaptureToolSuccessorServiceCached, codex0151C2PACaptureToolSuccessorServiceErr
}

func readCodex0151C2PACaptureToolSuccessorService() (codex0151ToolReadinessReceiptService, error) {
	var receipt codex0151ToolReadinessReceiptService
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex0151C2PACaptureToolSuccessorServicePath),
	))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateCodex0151C2PACaptureToolSuccessorService(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151C2PACaptureToolSuccessorService(receipt codex0151ToolReadinessReceiptService) error {
	...
	if receipt.Predecessor.Kind != "codex_cli_0151_tool_readiness_source_transition" ||
		receipt.Predecessor.Path != codex0151ToolReadinessTransitionServicePath ||
...
// codex0151C2PACaptureToolSuccessorSupersedesService 只承接 C2PA 工具后继的精确摘要边。
func codex0151C2PACaptureToolSuccessorSupersedesService(path, priorDigest, currentDigest string) bool {
	receipt, err := loadCodex0151C2PACaptureToolSuccessorService()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
...
func TestCodex0151C2PACaptureToolSuccessorSourceTransitionServiceIsFrozen(t *testing.T) {
	if _, err := loadCodex0151C2PACaptureToolSuccessorService(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151C2PACaptureToolSuccessorSourceTransitionServiceRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151C2PACaptureToolSuccessorService()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			mutated := receipt
			test.mutate(&mutated)
			if err := validateCodex0151C2PACaptureToolSuccessorService(mutated); err == nil {
