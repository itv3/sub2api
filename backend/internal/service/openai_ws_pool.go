...
func (c *openAIWSConn) supportsIdlePingWithoutReader() bool {
	...
	// 测试实现和替代实现沿用原有探测行为，除非它们明确声明该行为不安全。
...
func (p *openAIWSConnPool) prewarmConns(accountID int64, req openAIWSAcquireRequest, total int, generations ...uint64) {
	...
	defer func() {
		...
		if staleTarget {
			// 旧拨号进行时已有更新的获取请求；先清除 prewarmActive，再按最新的
			// beta、路由提示和传输画像重新补齐空闲连接预算。
			p.ensureTargetIdleAsync(accountID)
		}
	}()
...
// normalizeOpenAIWSAcquireCompatibility 将握手协商项与 Plus 运行时传输画像合并为
// 硬兼容键，避免跨 Host、代理、CA、TLS 画像或官方 ReleaseBundle 复用连接。
func normalizeOpenAIWSAcquireCompatibility(req openAIWSAcquireRequest) openAIWSHandshakeCompatibilityKey {
	...
	compatibility.transportKey = stringsTrim(req.TransportKey)
