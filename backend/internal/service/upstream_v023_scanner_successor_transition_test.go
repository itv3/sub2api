...
const upstreamV023ScannerSuccessorTransitionServicePath = "docs/egress/maintenance/upstream-v0.2.3-scanner-successor-source-transition.json"

type upstreamV023ScannerSuccessorPredecessorService struct {
	Kind   string `json:"kind"`
	Path   string `json:"path"`
	SHA256 string `json:"sha256"`
}

type upstreamV023ScannerSuccessorTransitionService struct {
	Path       string `json:"path"`
...
type upstreamV023ScannerSuccessorAdditionService struct {
	Path   string `json:"path"`
	SHA256 string `json:"sha256"`
	Reason string `json:"reason"`
}

type upstreamV023ScannerSuccessorSafetyService struct {
...
type upstreamV023ScannerSuccessorReceiptService struct {
	...
	Predecessor          upstreamV023ScannerSuccessorPredecessorService  `json:"predecessor"`
	Transitions          []upstreamV023ScannerSuccessorTransitionService `json:"transitions"`
	Additions            []upstreamV023ScannerSuccessorAdditionService   `json:"additions"`
	Verification         []string                                        `json:"verification"`
	Safety               upstreamV023ScannerSuccessorSafetyService       `json:"safety"`
	Result               string                                          `json:"result"`
	IdentitySHA256       string                                          `json:"identity_sha256"`
}

var (
	upstreamV023ScannerSuccessorServiceOnce    sync.Once
	upstreamV023ScannerSuccessorServiceCached  upstreamV023ScannerSuccessorReceiptService
	upstreamV023ScannerSuccessorServiceLoadErr error
)

func loadUpstreamV023ScannerSuccessorTransitionService() (upstreamV023ScannerSuccessorReceiptService, error) {
	upstreamV023ScannerSuccessorServiceOnce.Do(func() {
		upstreamV023ScannerSuccessorServiceCached,
			upstreamV023ScannerSuccessorServiceLoadErr = readUpstreamV023ScannerSuccessorTransitionService()
	})
	return upstreamV023ScannerSuccessorServiceCached, upstreamV023ScannerSuccessorServiceLoadErr
}

func readUpstreamV023ScannerSuccessorTransitionService() (upstreamV023ScannerSuccessorReceiptService, error) {
	var receipt upstreamV023ScannerSuccessorReceiptService
	raw, err := os.ReadFile(filepath.Join("../../..", filepath.FromSlash(upstreamV023ScannerSuccessorTransitionServicePath)))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateUpstreamV023ScannerSuccessorTransitionService(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateUpstreamV023ScannerSuccessorTransitionService(receipt upstreamV023ScannerSuccessorReceiptService) error {
	...
	predecessorDigest := upstreamMergeFrameworkServiceDigest(predecessorRaw)
	...
	for _, transition := range receipt.Transitions {
		...
		if readErr != nil || (upstreamMergeFrameworkServiceDigest(current) != transition.ToSHA256 &&
			!upstreamMergeFrameworkV3SuccessorSupersedesService(
				transition.Path, transition.ToSHA256, upstreamMergeFrameworkServiceDigest(current),
	...
	for _, addition := range receipt.Additions {
		...
		if readErr != nil || (upstreamMergeFrameworkServiceDigest(current) != addition.SHA256 &&
			!upstreamMergeFrameworkV3SuccessorSupersedesService(
				addition.Path, addition.SHA256, upstreamMergeFrameworkServiceDigest(current),
			)) {
...
func upstreamV023ScannerSuccessorTransitionSupersedesService(path, priorDigest, currentDigest string) bool {
	receipt, err := loadUpstreamV023ScannerSuccessorTransitionService()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
...
func TestUpstreamV023ScannerSuccessorTransitionServiceIsFrozen(t *testing.T) {
	if _, err := loadUpstreamV023ScannerSuccessorTransitionService(); err != nil {
		t.Fatal(err)
	}
}

func TestUpstreamV023ScannerSuccessorTransitionServiceRejectsMutation(t *testing.T) {
	receipt, err := loadUpstreamV023ScannerSuccessorTransitionService()
	if err != nil {
		t.Fatal(err)
	}
	mutated := receipt
	mutated.Transitions = append([]upstreamV023ScannerSuccessorTransitionService(nil), receipt.Transitions...)
	mutated.Transitions[0].ToSHA256 = strings.Repeat("0", 64)
	if err := validateUpstreamV023ScannerSuccessorTransitionService(mutated); err == nil {
