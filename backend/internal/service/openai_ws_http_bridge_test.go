...
func TestProxyOpenAIWSHTTPBridgeTurnOfficialEgressUsesHTTPProfile(t *testing.T) {
	...
	require.Equal(t, activeOpenAICodexTransportIDForTest(officialCodexEndpointResponsesHTTP), egressContext.TransportProfileID())
...
func TestProxyOpenAIWSHTTPBridgeTurnStagesMetadataBeforeCapacityFailover(t *testing.T) {
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"chatgpt_account_id": "chatgpt-account-12",
		},
	}
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
...
func TestProxyOpenAIWSHTTPBridgeTurnDoesNotReplayCapacityAfterSemanticOutput(t *testing.T) {
	...
	account := &Account{
		...
		Credentials: map[string]any{
			"access_token":       "sk-test",
			"chatgpt_account_id": "chatgpt-account-13",
		},
	}
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
