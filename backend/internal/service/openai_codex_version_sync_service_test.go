...
// 候选版本号优先级：管理员面板覆写 → 自动发现值 → 内置常量。
// 该优先级只用于发现和兼容展示，strict wire 激活仍由 ReleaseCatalog 独立控制。
...
// 候选 UA：面板未填完整 UA 时，按发现版本重建已验收 UA 模板的版本段。
...
func TestCodexDiscoveredVersionDoesNotActivateStrictWire(t *testing.T) {
	svc := NewSettingService(&codexVersionSettingRepoStub{values: map[string]string{
		SettingKeyOpenAICodexClientVersionSynced: "0.200.1",
	}}, nil)
	require.Equal(t, "0.200.1", svc.GetOpenAICodexClientVersion(context.Background()))
	require.Contains(t, svc.GetOpenAICodexCanonicalUserAgent(context.Background()), "/0.200.1 ")
...
// 回归：面板完整 UA 是唯一能改 OS / 架构 / 终端指纹的兼容输入，必须保留；但它填写于
// 某个历史版本，逐字沿用会让候选身份继续携带陈旧版本。因此只借它的指纹，候选版本段
// 按发现／管理员覆写值重建；strict wire 仍由 active ReleaseBundle 独立重建。
func TestGetOpenAICodexCanonicalUserAgentRebuildsPanelUAVersion(t *testing.T) {
	t.Run("陈旧面板 UA 跟随候选版本", func(t *testing.T) {
		svc := NewSettingService(&codexVersionSettingRepoStub{values: map[string]string{
