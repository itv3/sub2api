...
func TestNormalizeOpenAIWSPayloadWithoutInputAndPreviousResponseID(t *testing.T) {
	...
	require.True(t, gjson.GetBytes(normalized, "client_metadata").Exists())
	require.True(t, gjson.GetBytes(normalized, "stream_options").Exists())
	require.True(t, gjson.GetBytes(normalized, "generate").Exists())
	require.False(t, gjson.GetBytes(normalized, "generate").Bool())
...
func TestShouldKeepIngressPreviousResponseID(t *testing.T) {
	...
	t.Run("codex_prewarm_to_business_replays_when_non_input_changes", func(t *testing.T) {
		prewarmPayload := []byte(`{
			"type":"response.create",
			"model":"gpt-5.1",
			"store":false,
