...
const codex0151ContainerPathRecoveryToolSuccessorServicePath = "docs/egress/maintenance/codex-cli-0151-container-path-recovery-tool-successor-source-transition.json"

var (
	codex0151ContainerPathRecoveryToolSuccessorServiceOnce   sync.Once
	codex0151ContainerPathRecoveryToolSuccessorServiceCached codex0151ToolReadinessReceiptService
	codex0151ContainerPathRecoveryToolSuccessorServiceErr    error
)

func loadCodex0151ContainerPathRecoveryToolSuccessorService() (codex0151ToolReadinessReceiptService, error) {
	codex0151ContainerPathRecoveryToolSuccessorServiceOnce.Do(func() {
		codex0151ContainerPathRecoveryToolSuccessorServiceCached, codex0151ContainerPathRecoveryToolSuccessorServiceErr =
			readCodex0151ContainerPathRecoveryToolSuccessorService()
	})
	return codex0151ContainerPathRecoveryToolSuccessorServiceCached, codex0151ContainerPathRecoveryToolSuccessorServiceErr
}

func readCodex0151ContainerPathRecoveryToolSuccessorService() (codex0151ToolReadinessReceiptService, error) {
	var receipt codex0151ToolReadinessReceiptService
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex0151ContainerPathRecoveryToolSuccessorServicePath),
	))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateCodex0151ContainerPathRecoveryToolSuccessorService(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateCodex0151ContainerPathRecoveryToolSuccessorService(receipt codex0151ToolReadinessReceiptService) error {
...
// codex0151ContainerPathRecoveryToolSuccessorSupersedesService 只承接容器路径恢复工具后继的精确摘要边。
func codex0151ContainerPathRecoveryToolSuccessorSupersedesService(path, priorDigest, currentDigest string) bool {
	receipt, err := loadCodex0151ContainerPathRecoveryToolSuccessorService()
	if err != nil {
		return false
	}
	for _, transition := range receipt.Transitions {
...
func TestCodex0151ContainerPathRecoveryToolSuccessorSourceTransitionServiceIsFrozen(t *testing.T) {
	if _, err := loadCodex0151ContainerPathRecoveryToolSuccessorService(); err != nil {
		t.Fatal(err)
	}
}

func TestCodex0151ContainerPathRecoveryToolSuccessorSourceTransitionServiceRejectsMutation(t *testing.T) {
	receipt, err := loadCodex0151ContainerPathRecoveryToolSuccessorService()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			mutated := receipt
			test.mutate(&mutated)
			if err := validateCodex0151ContainerPathRecoveryToolSuccessorService(mutated); err == nil {
