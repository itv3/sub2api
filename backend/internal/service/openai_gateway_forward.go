...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	// 在分流到 passthrough、Codex transform 或原生 Chat Completions 前，
	// 按目标平台统一修正显式为 null 的工具 Schema type。
	...
	// OAuth 使用模型能力判定后的 useResponsesLite；API Key 使用客户端 Lite
	...
	// 记录并沿用上游的 invalid_encrypted_content lineage：同一会话后续
	// turn 先剥离已确认失效的密文，避免每轮重复触发拒绝。与 Plus 的
	// 作用域缓存同时保留，前者覆盖无完整作用域的会话恢复场景。
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, plan openAIUpstreamRequestPlan) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if plan.PromptCacheKey != "" {
			isolated := isolateOpenAIUpstreamSessionID(apiKeyID, codexAccountIdentitySource(c, account), plan.PromptCacheKey)
