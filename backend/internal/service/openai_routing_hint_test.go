			...
			for _, passthrough := range []bool{false, true} {
				...
				t.Run(mode, func(t *testing.T) {
					...
					if passthrough {
						...
						req, err = svc.buildUpstreamRequest(context.Background(), c, oauthAccount, tt.body, "test-token", openAIUpstreamRequestPlan{IsCodexCLI: true})
					}
...
func TestOpenAIRoutingDiagnosticsUseFinalDerivedValuesOnly(t *testing.T) {
	...
	_, err := svc.buildUpstreamRequest(context.Background(), c, account, body, "oauth-secret", openAIUpstreamRequestPlan{IsCodexCLI: true})
	require.NoError(t, err)
