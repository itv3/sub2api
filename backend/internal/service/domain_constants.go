...
const (
	...
	// SettingKeyOpenAICodexClientVersion 是管理员覆写的 Codex 候选版本号。
	// 空值表示跟随自动发现值；两者都没有结果时回退到内置常量。该值不直接激活 strict wire。
	SettingKeyOpenAICodexClientVersion = "openai_codex_client_version"
	// SettingKeyOpenAICodexClientVersionSynced 是自动发现任务写入的官方最新稳定版版本号。
	// 由 OpenAICodexVersionSyncService 独占写入，面板只读展示；管理员候选覆写请用
	// SettingKeyOpenAICodexClientVersion。
	SettingKeyOpenAICodexClientVersionSynced = "openai_codex_client_version_synced"
	// SettingKeyOpenAICodexVersionAutoSyncEnabled 是否启用 Codex 最新稳定版自动发现（默认 true）。
