...
func validateCodex0151ModelPolicyToolSuccessorService(receipt codex0151ToolReadinessReceiptService) error {
	...
	for _, transition := range receipt.Transitions {
		...
		if readErr != nil || (currentDigest != transition.ToSHA256 &&
			!codex0151ContainerPathRecoveryToolSuccessorSupersedesService(
				transition.Path,
				transition.ToSHA256,
				currentDigest,
			)) {
...
func codex0151ModelPolicyToolSuccessorSupersedesService(path, priorDigest, currentDigest string) bool {
	...
	return codex0151ContainerPathRecoveryToolSuccessorSupersedesService(path, priorDigest, currentDigest)
}
