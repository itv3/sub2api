...
func TestEnforceCodexIdentityHeadersWithAccountOverrideUA(t *testing.T) {
	...
	// 陈旧覆写 UA 跟随解析器发布的已验收版本，无需管理员重新编辑原配置。
	t.Run("陈旧覆写 UA 跟随已验收版本", func(t *testing.T) {
		SetCodexCanonicalUserAgentResolver(func() string {
			return "codex_exec/0.200.1 (Ubuntu 24.4.0; x86_64) unknown (codex_exec; 0.200.1)"
		})
