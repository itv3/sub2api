...
func newOpenAINamespaceTestService(upstream *httpUpstreamRecorder) *OpenAIGatewayService {
	svc := newOpenAIRejectedFieldTestService(upstream)
	// 本组测试验证非 Lite namespace 语义。显式冻结账号 manifest，既避免后台
	// /models 刷新抢占 recorder 响应，也避免随 bundled 快照演进改变测试前提。
	svc.openaiModelCapabilities.replaceFromManifest(
		5108,
		[]byte(`{"models":[{"slug":"gpt-5.5","visibility":"list","use_responses_lite":false,"supports_parallel_tool_calls":true}]}`),
	)
	return svc
}
...
func TestOpenAIGatewayService_ForwardClearsStaleNamespaceNames(t *testing.T) {
	...
	_, err := newOpenAINamespaceTestService(upstream).Forward(
		context.Background(), c, newOpenAIOAuthNamespaceTestAccount(), body,
	)
