...
const upstreamMergeFrameworkV4SuccessorServicePath = "docs/egress/maintenance/upstream-v0.2.3-framework-v3-successor.json"

type upstreamMergeFrameworkV4ServicePredecessor struct {
	Path   string `json:"path"`
	SHA256 string `json:"sha256"`
}

type upstreamMergeFrameworkV4ServiceSourceTransition struct {
	Path          string `json:"path"`
	SHA256        string `json:"sha256"`
...
type upstreamMergeFrameworkV4ServiceReceipt struct {
	...
	Predecessor          upstreamMergeFrameworkV4ServicePredecessor      `json:"predecessor"`
	SourceTransition     upstreamMergeFrameworkV4ServiceSourceTransition `json:"source_transition"`
...
var (
	upstreamMergeFrameworkV4ServiceOnce    sync.Once
	upstreamMergeFrameworkV4ServiceCached  upstreamMergeFrameworkV4ServiceReceipt
	upstreamMergeFrameworkV4ServiceLoadErr error
)

func loadUpstreamMergeFrameworkV4SuccessorService() (upstreamMergeFrameworkV4ServiceReceipt, error) {
	upstreamMergeFrameworkV4ServiceOnce.Do(func() {
		upstreamMergeFrameworkV4ServiceCached, upstreamMergeFrameworkV4ServiceLoadErr =
			readUpstreamMergeFrameworkV4SuccessorService()
	})
	return upstreamMergeFrameworkV4ServiceCached, upstreamMergeFrameworkV4ServiceLoadErr
}

func readUpstreamMergeFrameworkV4SuccessorService() (upstreamMergeFrameworkV4ServiceReceipt, error) {
	var receipt upstreamMergeFrameworkV4ServiceReceipt
	raw, err := os.ReadFile(filepath.Join("../../..", filepath.FromSlash(upstreamMergeFrameworkV4SuccessorServicePath)))
	if err != nil {
		return receipt, err
	}
	...
	if err := validateUpstreamMergeFrameworkV4SuccessorService(receipt); err != nil {
		return receipt, err
	}
	return receipt, nil
}

func validateUpstreamMergeFrameworkV4SuccessorService(receipt upstreamMergeFrameworkV4ServiceReceipt) error {
	...
	if receipt.Predecessor.Path != upstreamMergeFrameworkV3SuccessorServicePath ||
	...
	if err != nil || upstreamMergeFrameworkV3ServiceDigest(predecessorRaw) != receipt.Predecessor.SHA256 {
		return errors.New("上游合并框架 v0.2.3 successor 前序摘要不一致")
	}
	if receipt.SourceTransition.Path != upstreamV023SourceTransitionServicePath ||
		!validOpenAIReplayOOMRepairServiceSHA(receipt.SourceTransition.SHA256) ||
		receipt.SourceTransition.BaseCommit != receipt.BaseCommit ||
		!upstreamV023GitObjectService(receipt.SourceTransition.CurrentCommit) {
	...
	if err != nil || upstreamMergeFrameworkV3ServiceDigest(sourceRaw) != receipt.SourceTransition.SHA256 {
		return errors.New("上游合并框架 v0.2.3 successor 通用 transition 摘要不一致")
	}
	sourceReceipt, err := readUpstreamV023SourceTransitionService()
	...
	for _, transition := range receipt.Transitions {
		...
		if !ok || !slices.Equal(transition.PredecessorSHA256s, []string{from}) ||
			!validOpenAIReplayOOMRepairServiceSHA(transition.ToSHA256) || transition.ToSHA256 == from ||
		...
		if readErr != nil || (upstreamMergeFrameworkV3ServiceDigest(current) != transition.ToSHA256 &&
			!auditedSourceSuccessorReachesService(
				transition.Path, transition.ToSHA256, upstreamMergeFrameworkV3ServiceDigest(current),
			)) {
...
func upstreamMergeFrameworkV4SuccessorSupersedesService(path, priorDigest, currentDigest string) bool {
...
func TestUpstreamMergeFrameworkV4SuccessorServiceIsFrozen(t *testing.T) {
	if _, err := loadUpstreamMergeFrameworkV4SuccessorService(); err != nil {
		t.Fatal(err)
	}
}

func TestUpstreamMergeFrameworkV4SuccessorServiceRejectsMutation(t *testing.T) {
	receipt, err := loadUpstreamMergeFrameworkV4SuccessorService()
	if err != nil {
		t.Fatal(err)
	}
	mutated := receipt
	mutated.Transitions = append([]upstreamMergeFrameworkV3ServiceTransition(nil), receipt.Transitions...)
	mutated.Transitions[0].ToSHA256 = strings.Repeat("0", 64)
	if err := validateUpstreamMergeFrameworkV4SuccessorService(mutated); err == nil {
