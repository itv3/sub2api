...
var (
	// 共享默认身份只在启动时从 Active ReleaseCatalog 派生。Catalog 缺失或身份
	// 非法时直接终止启动，禁止回退到另一份可能漂移的版本常量。
	codexCLIUserAgent, codexCLIVersion = mustResolveActiveCodexIdentity()
)
...
type OpenAIGatewayService struct {
	...
	// 这是旧测试/兼容调用保留的账号级缓存；生产转发路径必须使用下面的
	// openaiInvalidEncryptedScopes，避免不同模型、协议和响应链之间互相污染。
	openaiInvalidEncryptedAccountsMu sync.Mutex
	openaiInvalidEncryptedAccounts   map[int64]openAIInvalidEncryptedAccountBinding
	openaiInvalidEncryptedScopes     map[openAIInvalidEncryptedScopeKey]openAIInvalidEncryptedAccountBinding
...
func (s *OpenAIGatewayService) writeOpenAIWSFallbackErrorResponse(c *gin.Context, account *Account, wsErr error) bool {
	...
	// 标记 committed：告知 handler 错误已完整写出，禁止再追加 SSE fallback。
