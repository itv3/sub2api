...
// AccountTestOptions 承载管理员连接测试的可选媒体与输出上限。
// ImageDataURL 与 AudioDataURL 必须是完整的 data URL。
...
// TestAccountConnection 通过真实测试请求检查账号连通性。
// modelID 可选；mode=compact 执行原生 remote compaction v2 探针，
// mode=official_files_probe 执行当前 Active Codex Files 三段式出站探针。
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, mode string, opts ...AccountTestOptions) error {
	...
	if account.IsOpenAI() {
		return s.testOpenAIAccountConnection(c, account, modelID, prompt, normalizeAccountTestMode(mode), testOpts.MaxOutputTokens)
	}
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string, maxOutputTokens ...int) error {
	...
	// 账号测试先应用普通模型映射；legacy compact 探针再叠加 compact 专用映射。
	testModelID = account.GetMappedModel(testModelID)
...
// testOpenAICompactConnection 使用当前严格画像已批准的 /responses/compact
// 管理端探针。上游原生 v2 的普通 /responses 行为需要独立 candidate 验收，
// 本次上游维护合并不得改变已冻结的业务 SinkID 与 wire。
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, credentialAccount *Account, testModelID string) error {
	...
	// 官方 Codex 的 compact 请求不发送入站 Accept；严格画像会按自身契约
	// 生成最终值，API Key mimic 也不得保留调用方协商头。
	...
	legacySucceeded := resp.StatusCode >= http.StatusOK && resp.StatusCode < http.StatusMultipleChoices
	if s.accountRepo != nil {
		updates := buildOpenAICompactProbeExtraUpdates(resp, body, nil, legacySucceeded, time.Now())
