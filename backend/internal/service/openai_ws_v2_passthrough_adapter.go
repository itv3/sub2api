...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	shouldNormalizeLitePayload := func(payload []byte) bool {
		...
		// API Key/Setup Token 没有 OAuth capability context，Lite 信号只存在于
		// 帧内 client_metadata；仍需走按账号类型分派的归一化，至少固定
		// parallel_tool_calls=false。
		return account.IsOpenAI() && isOpenAIResponsesLiteWebSocketPayload(payload)
	}
	if shouldNormalizeLitePayload(firstClientMessage) {
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate {
				if shouldNormalizeLitePayload(payload) {
