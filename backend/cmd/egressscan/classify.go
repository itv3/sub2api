...
var reviewedPostBootstrapInfrastructure = map[string]string{
	...
	"github.com/Wei-Shaw/sub2api/internal/service.*claudeHTTPUpstreamPort.SendHTTPUpstream@backend/internal/service/official_egress_1b_executor.go#facade_http_upstream_do_tls#1":                      "FW-H Claude Candidate／Production 共用 Executor 的隔离 HTTPUpstream terminal delegation；业务 SinkID、画像、TLS 与定型凭证必须来自 Claude Executor Plan",
...
var classifyRules = []classifyRule{
	...
	{
		funcExact: "*claudeHTTPUpstreamPort.SendHTTPUpstream",
		persona:   "infrastructure", backend: "http_upstream", state: "not_applicable",
		owner: ownerEgress, changeset: "FW-H",
		rationale: "Claude Candidate／Production 共用 Executor adapter 的隔离 terminal delegation；业务 SinkID、画像、TLS 与 FinalizationToken 均由上游 Executor 冻结",
	},
	{
